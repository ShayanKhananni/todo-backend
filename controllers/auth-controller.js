...
export const signinGoogle = async (req,res,next) =>
{
  ...
  try {
    ...
    {
      const password = bycrypt.hashSync((Math.random().toString(36).slice(-8) + Math.random().toString(36).slice(-8)));
      const username = displayName.split(" ").join("_").toLowerCase() + Math.floor(Math.random() * 10000 + 1 );
